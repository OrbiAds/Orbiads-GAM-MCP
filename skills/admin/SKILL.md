---
name: orbiads-admin
description: Use for GAM network administration — teams, sites, labels, custom fields, naming conventions, delivery defaults, and network settings — through OrbiAds MCP actions or their orbiads CLI equivalents. Examples: "create a team", "add a label", "set the naming convention". Enforces preview-before-write and billing-guard rules.
version: 1.0.0
author: OrbiAds
license: MIT
metadata:
  group: admin
  tags: [orbiads, gam, adops, admin, settings]
...
# OrbiAds GAM — Network Administration & Settings

_Manage global settings, naming conventions, delivery defaults, network configurations, teams, sites, labels, and custom fields._

**Mode:** mixed (read + write) · **Tools included:** 5 · **Total actions:** 100 · **CLI coverage:** partial (13/100 actions)

Parent MCP tools: `gam_admin`, `gam_features`, `network`, `settings`, `tenant_catalog`.

## Surfaces

OrbiAds exposes the same backend through two surfaces; **MCP is the default**. A subset of actions also has a CLI command (`cli_coverage: partial` — 13/100 actions here). Per-action availability (MCP command vs `orbiads <command>` vs `MCP-only`) is listed in [`references/actions.md`](references/actions.md).
