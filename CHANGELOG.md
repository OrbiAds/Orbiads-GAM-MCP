...
Replaces the flat catalogue (8 shared skills + ~270 child tools at v1.0.0) with **36 parent tools** that dispatch all operations via an `action` discriminator. The 241 pre-refactor child tools remain available as soft-deprecated wrappers that route to their parent (emit `deprecated_tool_called` analytics).
...
- **Epic 82**: `gam_jobs`
- **Epic 98**: `dai_skill`, `live_stream`, `video_ops`, `yield_skill`

### Largest parent tools (by action count)

| Parent | Actions | Mode |
|---|---|---|
| `gam_admin` | 61 | mixed (read + write) |
| `reporting` | 32 | mixed (read + write) |
| `creatives` | 30 | mixed (read + write) |
| `deals` | 28 | mixed (read + write) |
| `targeting` | 28 | mixed (read + write) |
| `settings` | 26 | mixed (read + write) |
...
Replaces the flat catalogue (8 shared skills + ~270 child tools at v1.0.0) with **36 parent tools** that dispatch all operations via an `action` discriminator. The 237 pre-refactor child tools remain available as soft-deprecated wrappers that route to their parent (emit `deprecated_tool_called` analytics).
...
| Parent | Actions | Mode |
|---|---|---|
| `gam_admin` | 58 | mixed (read + write) |
| `reporting` | 31 | mixed (read + write) |
| `creatives` | 30 | mixed (read + write) |
| `deals` | 28 | mixed (read + write) |
| `targeting` | 27 | mixed (read + write) |
| `creative_assets` | 22 | mixed (read + write) |
| `blueprint` | 20 | mixed (read + write) |
| `line_items` | 16 | mixed (read + write) |
