...
3. Resolve advertiser, order, and line-item context through the `campaigns` skill: `companies(area="advertisers", action="find_or_create")`, `orders(action="find_or_create")`, `line_items(action="create_batch")`.
