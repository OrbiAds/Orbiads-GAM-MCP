...
4. Use the `campaigns` skill to resolve or create the advertiser via `companies(area="advertisers", action="find_or_create")`, the order via `orders(action="find_or_create")`, and target line items via `line_items(action="create_batch")`.
